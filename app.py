...
simbol = st.sidebar.text_input(
    "Masukkan simbol saham (contoh: BBCA.jk catatan: tambahkan simbol .jk setelah simbol jika ingin menampilkan saham indonesia):",
    "NVDA")
...
if data.empty:
    st.error(
        "Tidak ada data yang tersedia untuk simbol saham dan rentang tanggal yang dipilih. Harap sesuaikan masukan.")
else:
    ...
    # Updated: Calculate RMSE using root_mean_squared_error
    rmse = mean_squared_error(df_eval['y'], df_eval['yhat'])
    rmse = rmse ** 0.5  # Take the square root to get the root mean squared error
